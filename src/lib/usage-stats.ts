...
export async function getSignupStats(): Promise<SignupStats> {
  ...
  if (error) {
    ...
    return { total_users: 0, signups_by_month: [] }
  }
...
export async function getChatStats(): Promise<ChatStats> {
  ...
  if (chatError) {
    ...
    return { total_chats: 0, chats_by_day: [], custom_count: 0, prewritten_count: 0 }
  }

  // Get all suggestions to compare
  ...
  for (const chat of chats || []) {
    // Classify as prewritten or custom
...
export async function getSummaryStats(): Promise<SummaryStats> {
  ...
  if (error) {
    ...
    return { total_summaries: 0, by_style: [], by_length: [] }
  }
